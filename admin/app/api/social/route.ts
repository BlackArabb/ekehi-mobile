...
export async function POST(request: Request) {
  try {
    ...
    const transformedResult = {
      ...
      taskType: (result as any).taskType || 'generic',
      rewardCoins: (result as any).rewardCoins || 0,
      actionUrl: (result as any).actionUrl || null,
      verificationMethod: (result as any).verificationMethod || 'manual',
      verificationData: (result as any).verificationData || null,
      isActive: (result as any).isActive || false,
      sortOrder: (result as any).sortOrder || 0,
